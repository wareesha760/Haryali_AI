...
function Shop({ title, image, description, url = "", subscription }) {
  ...
  return (
    ...
    >
      {isDisabled ? (
        <div
          className={`rounded-2xl shadow-xl border border-white/30 overflow-hidden transition-all duration-300 w-80 h-[380px] opacity-50 grayscale pointer-events-none ${cardStyle}`}
        >
          <motion.img
            src={image}
            alt={title}
