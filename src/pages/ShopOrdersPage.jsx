...
export default function ShopOrdersPage() {
  ...
  // ✅ Fetch shop orders from backend API
  useEffect(() => {
    const fetchShopOrders = async () => {
      const token = localStorage.getItem("token");
      if (!token) {
        setError("Please login to view your shop orders");
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        // Filter only shop orders
        const shopOrders = (data.orders || data).filter(order => order.orderType === 'shop');
        console.log("Filtered shop orders:", shopOrders);
  ...
  // ✅ Delete Shop Order from backend
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-xl text-green-600">Loading shop orders...</div>
      </div>
    );
  }
      ...
      >
        {orders.length === 0 ? (
          <p className="text-gray-500 text-center text-lg">
            ابھی تک کوئی دکان کا آرڈر نہیں کیا گیا۔
          </p>
        ) : (
